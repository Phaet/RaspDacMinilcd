...
## Summary ##
- This branch contains a version of the RaspDacMiniLCD package for moOde9
...
## Installation ##
This walkthrough assumes you are working on a fresh moOde 9 installation with a wired network.
I did test on a pi4 so far. It is very likely that it will work on a pi3 as well.

### 1. Connect to your RDMLCD using SSH ###
Please refer to this [guide](https://github.com/moode-player/docs/blob/main/setup_guide.md) if you have trouble setting up your moOde install with SSH enabled.
Once you are in, start by updating apt
```
...
This is the main part that replaces fbcp-ili9341, you can inspect the source file [here](https://github.com/audiophonics/RaspDacMinilcd/blob/moode9/usr/local/share/rdm/dtoverlay/ili9341.dts).
Be careful as this will write into /boot/firmware/config.txt and add a dts file into /boot/firmware/overlays/
```
...
bc is needed to allow cycling alsa properties from within [apessq2m shell script](https://github.com/audiophonics/RaspDacMinilcd/blob/moode9/usr/local/bin/apessq2m) 
```
