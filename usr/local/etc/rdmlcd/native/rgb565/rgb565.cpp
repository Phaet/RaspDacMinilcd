...
for (size_t i = 0, j = (outputLength / 2) - 1; i < length; i += 4, j--) {
    uint8_t r = inputData[i];
    uint8_t g = inputData[i + 1];
    uint8_t b = inputData[i + 2];

    uint16_t rgb565 = (r >> 3 ) | (g >> 2 << 5) | (b >> 3 << 11);
    // Ensure j doesn't go below 0 to prevent underflow
    if (j < outputLength) {
        outputData[j] = htons(rgb565); // Use htons to ensure correct byte order
    }
}
    return output;
